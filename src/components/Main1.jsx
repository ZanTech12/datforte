...
function Main1() {
  // --- KEY FIX: Set initial state to `true` ---
  // This ensures content is visible by default, even if JS is disabled.
  const [isVisible, setIsVisible] = useState(true);
  const [showAdmissionAlert, setShowAdmissionAlert] = useState(true);

  useEffect(() => {
    // This effect is now only for the alert, as `isVisible` is already true.
      ...
      <div className="container pb-5 w-full grid grid-cols-1 lg:grid-cols-1 mt-5 relative">
        ...
        <div className={`bg-white bg-opacity-90 backdrop-blur-sm rounded-xl shadow-lg lg:p-[3rem] p-5 container lg:w-[1000px] border border-gray-100 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className={`mt-8 flex justify-center transition-all duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`} style={{ transitionDelay: '500ms' }}>
            <button
              onClick={() => {
                alert('📢 ADMISSION INFORMATION 📢\n\nThank you for your interest in Datforte International Schools!\n\nOur admission process is simple and straightforward:\n\n1. Obtain an application form from our school office or website\n2. Submit the completed form with required documents\n3. Schedule an entrance examination\n4. Attend an interview with our admissions team\n5. Receive admission decision and complete registration\n\nFor more information, please contact our admissions office:\n📞 Phone: +234 123 456 7890\n📧 Email: admissions@datforte.edu.ng\n📍 Address: 14, Ahmed Tijani St, Ahmadiya Bus-Stop, Lagos');
              }}
